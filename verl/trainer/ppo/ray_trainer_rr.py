...
class RayPPOTrainer:
    ...
    #             target_batch.append(dict(problem=problem, response=response, score=bool(reward_value), correct_ratio=correct_ratio, data_source=data_source))
    #             count += 1
    ...
    def reward_shaping(self, batch: DataProto) -> DataProto:
        """
        对批次数据进行奖励整形和裁剪。
        此版本通过在每个问题（uid）内部进行指定裁剪，将“过度表现”的小组
        （成功率过高或过低）改造为“优质小组”（成功率在目标范围内），
        从而最大化数据利用率和学习信号。
        """
        TARGET_RATIO_MIN = 0.25
        TARGET_RATIO_MAX = 0.75
        # 为了最大化梯度信号，我们将改造后的目标成功率设为 0.5
        ...
        for uid in unique_uids:
            ...
            # 分离成功和失败轨迹的索引
            success_indices = uid_indices[uid_scores == 1]
            failure_indices = uid_indices[uid_scores == 0]

            if TARGET_RATIO_MIN <= correct_ratio <= TARGET_RATIO_MAX:
                # Case 1: 本身就是“优质小组”，保留所有轨迹
                final_keep_indices.extend(uid_indices)
            
            elif correct_ratio > TARGET_RATIO_MAX:
                # Case 2: “过易小组”，成功率太高。我们需要丢弃一部分成功轨迹
                num_failures = num_rollouts - num_success
                num_success_to_keep = int(np.round((TARGET_RATIO_MAX * num_failures) / (1 - TARGET_RATIO_MAX)))
                
                if num_success > 0 and num_success_to_keep == 0:
                    num_success_to_keep = 1

                kept_success_indices = np.random.choice(success_indices, size=num_success_to_keep, replace=False)
                
                final_keep_indices.extend(failure_indices)
                final_keep_indices.extend(kept_success_indices)

            else: # correct_ratio < TARGET_RATIO_MIN
                # Case 3: “过难小组”，成功率太低。我们需要丢弃一部分失败轨迹
                num_failures_to_keep = int(np.round((num_success * (1 - TARGET_RATIO_MIN)) / TARGET_RATIO_MIN))

                num_failures = num_rollouts - num_success
                if num_failures > 0 and num_failures_to_keep == 0:
                    num_failures_to_keep = 1

                if num_failures > 0:
                    kept_failure_indices = np.random.choice(failure_indices, size=min(num_failures_to_keep, num_failures), replace=False)
                    final_keep_indices.extend(kept_failure_indices)
                
                final_keep_indices.extend(success_indices)

        # ==================== 新增代码开始 ====================
        # 确保最终的 batch 大小可以被 8 整除
        
        # 首先将列表转换为 numpy 数组以便进行高效操作
        final_keep_indices_arr = np.array(final_keep_indices)
        current_size = len(final_keep_indices_arr)

        # 如果当前样本数不足8，则可能无法处理，可以选择返回一个空batch或抛出异常
        # 这里我们选择如果样本太少就直接返回空的，避免下游出错
        if current_size < 8:
            # 返回一个空的 batch
            return batch.select_idxs(np.array([]))

        # 计算需要丢弃多少个样本才能使总数成为 8 的倍数
        num_to_discard = current_size % 8
        
        if num_to_discard > 0:
            # 如果需要丢弃，计算裁剪后的目标大小
            target_size = current_size - num_to_discard
            # 从要保留的索引中，随机选择 target_size 个样本。
            # 这比简单地从末尾删除更公平，避免了引入偏差。
            final_indices_to_use = np.random.choice(final_keep_indices_arr, size=target_size, replace=False)
        else:
            # 如果已经是 8 的倍数，则无需更改
            final_indices_to_use = final_keep_indices_arr
            
        # 对索引进行排序，以便在某些框架中进行更高效的切片操作
        keep_indices = np.sort(final_indices_to_use)
        # ==================== 新增代码结束 ====================
